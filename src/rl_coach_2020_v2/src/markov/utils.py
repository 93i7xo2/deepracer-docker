...
def copy_best_frozen_model_to_sm_output_dir(s3_bucket, s3_prefix, region,
    ...
    self = copy_best_frozen_model_to_sm_output_dir
    ...
    if best_checkpoint_num_s3 == -1 or len(dest_dir_pb_files) == 0:
        ...
        if not os.path.isfile(os.path.join(source_dir, best_model_name)):
            ...
            if self.pretrained_model is not None:
                print('DEBUG: model not found during training: {}'.format(best_model_name))
                print('DEBUG: use pretrianed model instead: {}'.format(self.pretrained_model))
                # download pretrained model
                download_frozen_model_from_minio(s3_bucket,
                                                 s3_prefix,
                                                 region,
                ...
                self.pretrained_model = best_model_name

                print('DEBUG: using pretrained model: {}'.format(best_model_name))
                print('DEBUG: model does not exist, download from minio: {}'.format(os.path.join(source_dir, best_model_name)))
                
                # download the best model from minio
                download_frozen_model_from_minio(s3_bucket,
                                                s3_prefix,
                                                region,
                                                best_model_name,
...
def download_frozen_model_from_minio(s3_bucket, s3_prefix, region, best_model_name, s3_endpoint_url, save_model_name=None):
    try:
        ...
        deepracer_model = os.path.join(os.getcwd(), './frozen_models/agent', save_model_name)
